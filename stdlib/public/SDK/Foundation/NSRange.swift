...
extension NSRange {
  ...
  where R.Bound == String.Index, S.Index == String.Index {
    let range = region.relative(to: target)
    let start = range.lowerBound.samePosition(in: target.utf16)
    let end = range.upperBound.samePosition(in: target.utf16)
    let location = target.utf16.distance(
      from: target.utf16.startIndex, to: start)
    let length = target.utf16.distance(from: start, to: end)
