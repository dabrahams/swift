...
@_versioned
class _StringStorage<Element: UnsignedInteger>
  : _SwiftNativeNSString, // Dynamically provides inheritance from NSString
    ...
    FactoryInitializable     // Factory constructor hack
{
  @nonobjc
  ...
  internal convenience init(
    ...
    // above to keep the logic in an init.
    self.init(
    ...
    withUnsafeMutablePointer(to: &_header) {
      $0.initialize(
        to: _SwiftStringBodyStorage(
          count: numericCast(count),
          capacity: numericCast(allocatedCapacity()), flags: 0))
    }
  }
...
final class _UTF16StringStorage : _StringStorage<UTF16.CodeUnit> {
