...
protocol _TailAllocated1 : class, FactoryInitializable {
  associatedtype Element
  associatedtype Header
  var _header: Header { get set }
}

extension _TailAllocated1 {
  @nonobjc
  init(
    minimumCapacity: Int,
    makeInitialHeader: (_ allocatedCapacity: Int)->Header
  ) {
    // This is effectively an assignment to self, using FactoryInitializable
    // to keep the logic in an init.
    self.init(
      Builtin.allocWithTailElems_1(
        ...
        Element.self))
/*
    // Find out how much capacity we actually got
    ...
    let allocatedCapacity = endAddr.assumingMemoryBound(to: Element.self)
...
@_versioned
class _StringStorage<Element_: UnsignedInteger>
    ...
    _TailAllocated1
{
  ...
  convenience init(
    ...
    self.init(minimumCapacity: Swift.max(count, minimumCapacity)) {
      _SwiftStringBodyStorage(
          count: numericCast(count),
          capacity: numericCast($0), flags: 0)
    }
  }
